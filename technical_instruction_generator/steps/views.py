...
class FullView(View):
    def get_group(self) -> SizedGroup:
        view_box = ViewBox.combine([step.view_box for step in self.steps])
        group = SizedGroup(width=view_box.width, height=view_box.height, flip_y=True)
...
class CloseUpView(View):
    def __init__(self, *args, padding: int | tuple[int, int] = 0, **kwargs):
        ...
        # initialize ViewBox
        view_box = ViewBox.combine([step.view_box_closeup for step in self.steps])
